...
namespace Hotel_Application.Services.Implementation
{
    public class UserService: IUserService
    {
        ...
        public async Task<ChangePasswordResult> ChangePassword(long userId, ChangePasswordViewModel changePassword)
        {
            ...
            if (user != null)
            {
                var currentPassword = PasswordHelper.EncodePasswordMd5(changePassword.CurrentPassword);

                if (user.Password == currentPassword)
                {
                    var newPassword = PasswordHelper.EncodePasswordMd5(changePassword.NewPassword);
